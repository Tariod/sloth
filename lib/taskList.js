...
class TaskList {
  constructor(data = [], key = 'dueDate', isMax = false) {
    this.isEqlProcessingTime = true;

    for (let i = 0; i < data.length; i++) {
      const task = data[i];
      if (!TaskList.validate(task)) throw new Error('TypeError');
      if (data[0].processingTime !== task.processingTime)
        this.isEqlProcessingTime = false;
    }
  ...
  insert(value) {
    if (TaskList.validate(value)) {
      if (
        this.heap.heap.length > 1 &&
        this.heap.heap[0].processingTime !== value.processingTime
      )
        this.isEqlProcessingTime = false;
  ...
  add(value) {
    if (TaskList.validate(value)) {
      if (this.heap.heap[0].processingTime !== value.processingTime)
        this.isEqlProcessingTime = false;
